...
CREATE INDEX `idx_agent_messages_convo` ON `agent_messages` (`conversation_id`);
...
CREATE UNIQUE INDEX `uq_forecast_bin` ON `bin_forecasts` (`bin_id`);
...
CREATE INDEX `idx_readings_time` ON `bin_sensor_readings` (`recorded_at`);
CREATE TABLE `bins` (
...
CREATE UNIQUE INDEX `bins_bin_code_unique` ON `bins` (`bin_code`);
CREATE INDEX `idx_bins_ward` ON `bins` (`ward_id`);
...
CREATE INDEX `idx_collection_bin_time` ON `collection_history` (`bin_id`,`collected_at`);
...
CREATE UNIQUE INDEX `complaints_complaint_code_unique` ON `complaints` (`complaint_code`);
...
CREATE UNIQUE INDEX `municipal_staff_employee_no_unique` ON `municipal_staff` (`employee_no`);
...
CREATE INDEX `idx_notifications_recipient` ON `notifications` (`recipient_user_id`,`delivery_status`);
...
CREATE UNIQUE INDEX `routes_route_code_unique` ON `routes` (`route_code`);
CREATE INDEX `idx_routes_ward_status` ON `routes` (`ward_id`,`status`);
...
CREATE UNIQUE INDEX `truck_drivers_license_no_unique` ON `truck_drivers` (`license_no`);
...
CREATE UNIQUE INDEX `trucks_plate_number_unique` ON `trucks` (`plate_number`);
CREATE TABLE `users` (
...
CREATE UNIQUE INDEX `ward_officers_employee_no_unique` ON `ward_officers` (`employee_no`);
CREATE TABLE `wards` (
