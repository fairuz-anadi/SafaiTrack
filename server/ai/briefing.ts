...
- Never use the words "proven", "optimal", "shortest", "guaranteed", "prevented", "actual savings", "real savings", "measured savings", or "confirmed savings". Never mention traffic, road conditions or whether a load fits a truck. Never tell anyone to dispatch, send, cancel or reassign anything. Do not write negative caveats containing these words (e.g. do not write "not proven"); use noteIds instead.
...
- Each section cites up to 8 primary evidenceIds that its sentences rest on. noteIds carries at most 3 items. cannotAnswer carries at most 3 items.`;
...
Never use the word "proven" in any context (not even "not proven" or "unproven"). Say "planning estimate" or "not confirmed in the field" instead.`,
...
export async function explainSnapshot(
  ...
  try {
    ...
    const focused = snapshot.focus
      ...
      : "";
    const system = `${COMMON_PROMPT}\n\n${FEATURE_PROMPT[snapshot.feature]}${focused}\n\nWrite every word of headline, summary and section text in ${LANGUAGE_NAME[language]}. For Bangla, write the everyday Bangla a municipal supervisor would speak, not a literal translation of English technical phrasing; keep digits in Western form, and write all counts as digits (e.g. 2টি, 4টি, 32টি), never as spelled-out words like দুটি, তিনটি, চারটি.`;
    ...
    try {
      ...
      console.warn("[briefing] validation rejected:", err instanceof Error ? err.message : err);
